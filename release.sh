#!/bin/bash
# Hawky を1つ上げる。DMG を作り、更新情報に署名して、GitHub Releases に置く。
#
#   ./release.sh 0.1.0
#
...
VERSION="${1:-}"
if [ -z "$VERSION" ]; then
  echo "使い方: ./release.sh <版数>   例: ./release.sh 0.1.0" >&2
  exit 1
fi

REPO="piro0919/hawky"
APP="Hawky.app"
DMG="Hawky-${VERSION}.dmg"
ZIP="Hawky-${VERSION}.zip"

# 版数を Info.plist に焼き込むため、build.sh へ渡す
HAWKY_VERSION="$VERSION" ./build.sh
./"$APP"/Contents/MacOS/Hawky --selftest
...
hdiutil create -quiet -volname "Hawky" -srcfolder "$STAGE" -ov -format UDZO "dist/$DMG"
rm -rf "$STAGE"
...
echo "DMG の sha256（Homebrew の cask に書く）:"
